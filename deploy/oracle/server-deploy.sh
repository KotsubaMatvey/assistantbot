...
if [ -z "${BOT_TOKEN:-}" ] || [ -z "${DOMAIN:-}" ] || [ -z "${ADMIN_TELEGRAM_IDS:-}" ] || [ -z "${ADMIN_BACKUP_ENCRYPTION_KEY:-}" ]; then
  echo "BOT_TOKEN, DOMAIN, ADMIN_TELEGRAM_IDS and ADMIN_BACKUP_ENCRYPTION_KEY are required in deploy-secrets.env." >&2
  exit 1
fi
...
ASSISTANT_ACCESS_MODE=admin_only
...
ADMIN_BACKUP_ENCRYPTION_KEY=${ADMIN_BACKUP_ENCRYPTION_KEY}
EOF
}
