...
Прототип интерфейса лежит в `miniapp/`. Это статический frontend для будущего пульта бота:
покупки, рынки, ассистент и память. Чтобы подключить его к Telegram:
...
payload для сравнения корзины и команд `markets`, `status`, `agenda`, `compact`, `new`.
