...
def help_text(*, include_admin: bool = False, settings: Settings | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "Быстрый поток:",
        "1. Пиши как в диалоге; обычное сообщение идёт в свободный разговор "
        "и не сохраняется в память автоматически.",
        "2. Для записи и действий формулируй намерение: «запомни ...», «напомни завтра в 9», "
        "«потратил 740 на продукты», «получил 50000 зарплата», «добавь молоко в покупки». "
        "Подсказка всегда доступна фразой «что ты умеешь».",
        "3. Уточняй и исправляй: «забудь это», «не расход, а перевод», "
        "кнопки отмены и редактирования.",
        "4. Спрашивай память: «что мама предпочитает?», «что у меня сегодня?».",
        "5. Проверяй изменения: «покажи, что ты сегодня запомнил», "
        "«покажи историю изменений», «отмени последнюю запись».",
        "6. Голос и фото принимаются; без STT/OCR бот попросит текстовую расшифровку.",
        "7. Используй /status, /new, /compact, /mode для operator-контроля сессии.",
        mini_app_hint.replace("4.", "8."),
        "",
        "Команды:",
    ]
    commands = [
