...
def help_text(*, include_admin: bool = False, settings: Settings | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "Быстрый поток:",
        "1. Пиши как в диалоге: «напомни завтра в 9», «потратил 740 на продукты», "
        "«добавь молоко в покупки».",
        "2. Уточняй и исправляй: «забудь это», «не расход, а перевод», "
        "кнопки отмены и редактирования.",
        "3. Спрашивай память: «что мама предпочитает?», «что у меня сегодня?».",
        "4. Проверяй изменения: «покажи, что ты сегодня запомнил», "
        "«покажи историю изменений», «отмени последнюю запись».",
        "5. Голос и фото принимаются; без STT/OCR бот попросит текстовую расшифровку.",
        "6. Используй /status, /new, /compact, /mode для operator-контроля сессии.",
        mini_app_hint.replace("4.", "7."),
        "",
        "Команды:",
    ]
    commands = [
