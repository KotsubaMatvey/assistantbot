...
def test_refresh_prices_for_items_calls_selected_stores(monkeypatch) -> None:
    ...
    async def fake_refresh(store_slug: str, queries: list[str]) -> StoreRefreshResult:
        calls.append((store_slug, queries))
    ...
    assert calls == [
        ("smart", ["молоко 1 л", "сахар 1 кг"]),
        ("magnit", ["молоко 1 л", "сахар 1 кг"]),
    ]
